...
from util import make_cc_embedding
...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    if FLAGS.mode == 'decode':
      max_dec_steps_value = hps.max_dec_steps
    else:
      max_dec_steps_value = hps.max_dec_steps.value

    self._dec_batch = tf.placeholder(tf.int32, [hps.batch_size.value, max_dec_steps_value], name='dec_batch')
    self._target_batch = tf.placeholder(tf.int32, [hps.batch_size.value, max_dec_steps_value], name='target_batch')
    self._dec_padding_mask = tf.placeholder(tf.float32, [hps.batch_size.value, max_dec_steps_value], name='dec_padding_mask')
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      weights = make_cc_embedding(self._vocab)

      weights_initializer = tf.constant_initializer(weights)

      with tf.variable_scope('embedding'):
        embedding = tf.get_variable('embedding', [vsize, hps.emb_dim.value], dtype=tf.float32, initializer=weights_initializer)
